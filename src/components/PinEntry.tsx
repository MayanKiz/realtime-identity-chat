...
const PinEntry = ({ onAccess, onAdminAccess, onCreateIdentity }: PinEntryProps) => {
  ...
  return <section className="akgec-landing page-scroll">
    ...
    {showGuide && <div className="akgec-guide-backdrop" role="dialog" aria-modal="true" aria-labelledby="guide-title" onClick={() => setShowGuide(false)}><div className="akgec-guide" onClick={(event) => event.stopPropagation()}><button className="guide-close" onClick={() => setShowGuide(false)} aria-label="Close tutorial"><X /></button><span className="akgec-label">QUICK START GUIDE</span><h2 id="guide-title">Welcome to AKGEC Community.</h2><p className="guide-intro">Sample Message</p><div className="guide-steps"><article><span className="guide-number">01</span><div><Users /><h3>Create your identity</h3><p>Sample Message</p></div></article><article><span className="guide-number">02</span><div><MessageCircle /><h3>Join the community</h3><p>Enter your Secret PIN to open the dashboard and talk with registered AKGEC members in the community room.</p></div></article><article><span className="guide-number">03</span><div><Search /><h3>Start private chats</h3><p>Use the search gateway with someone’s Chat PIN. There are no contact lists or public directories.</p></div></article></div><div className="guide-features"><span><LockKeyhole />Sample Message</span><span><Crown /> visible rank hierarchy</span><span><ShieldCheck /> nickname-first privacy</span></div><button className="akgec-primary guide-cta" onClick={() => { setShowGuide(false); onCreateIdentity(); }}>Create my account <ArrowRight /></button></div></div>}
  </section>;
};
