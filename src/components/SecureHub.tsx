...
const SecureHub = ({ currentUser, onBack, onDeleteAccount }: SecureHubProps) => {
  ...
  return <section className="secure-shell">
    <aside className="secure-sidebar"><div className="secure-brand"><span className="secure-brand-mark"><ShieldCheck /></span><span>CAMPUS TRADE</span></div><div className="sidebar-status"><span className="status-dot" /> you are online</div><nav className="secure-nav" aria-label="Messages navigation"><button className={`nav-item ${room === 'community' ? 'active' : ''}`} onClick={() => setRoom('community')}><Hash /> Community room <span>01</span></button><button className={`nav-item ${room === 'private' ? 'active' : ''}`} onClick={() => target && setRoom('private')} disabled={!target}><MessageCircle /> Private channel <span>{target ? '01' : '—'}</span></button></nav><div className="sidebar-note"><LockKeyhole /><p>Real people.<br />Thoughtful spaces.<br />Messages in realtime.</p></div><div className="sidebar-bottom"><button className="sidebar-action" onClick={onBack}><LogOut /> Back to workspace</button><button className="sidebar-action danger" onClick={deleteAccount}><Trash2 /> Delete identity</button></div></aside>
    <main className="secure-main"><header className="secure-topbar"><button className="mobile-back" onClick={onBack} aria-label="Back to workspace"><ArrowLeft /></button><div><p className="eyebrow">CAMPUS CONVERSATIONS / 2026</p><h1>Stay in the loop.</h1></div><div className="topbar-privacy"><LockKeyhole /> THOUGHTFUL BY DESIGN</div></header>
